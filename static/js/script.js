...
let settings = {
  ...
  color: 0x2a5784,
  smoothingLevel: 1,
  normalize: true  // Auto-normalize displacement data to [-1, 1] range
};
...
function normalizeDisplacements(displacements) {
  ...
  console.log(`Data normalized: min=${min.toFixed(2)}, max=${max.toFixed(2)}, range=${range.toFixed(2)}`);
...
function applyBackendDisplacement(data) {
  ...
  const normalizedDisplacements = settings.normalize 
    ? normalizeDisplacements(data.displacements)
    : data.displacements;
  ...
  if (backendData.stats) {
    updateDataStats(backendData.stats);
  }
...
async function fetchTopologyData() {
  ...
  if (!url) {
    alert('Please enter a backend endpoint URL.');
    return;
  }
  ...
  try {
    const requestData = {
      ...
      lod_level: Math.log2(settings.resolution / 32)
    };
    ...
    let msg = `Data loaded: ${backendData.metadata?.vertex_count?.toLocaleString() ?? 'N/A'} vertices`;
    
    if (backendData.stats) {
      msg += `\nData range: ${backendData.stats.min.toFixed(2)} to ${backendData.stats.max.toFixed(2)}`;
      msg += `\nNormalized: ${settings.normalize ? 'Yes' : 'No'}`;
    }
...
function setupEventListeners() {
  ...
  document.getElementById('fetch-btn').addEventListener('click', fetchTopologyData);
  ...
  // Add normalize toggle handler if it exists
  const normalizeToggle = document.getElementById('normalize-toggle');
  if (normalizeToggle) {
    normalizeToggle.addEventListener('change', onNormalizeToggle);
  }
}
...
function onNormalizeToggle(e) {
  settings.normalize = e.target.checked;
  
  // Reapply displacement with new normalization setting
  if (backendData) {
    applyBackendDisplacement(backendData);
  }
}
...
function updateDataStats(stats) {
  ...
  if (statsEl) {
    statsEl.innerHTML = `
      ...
      Range: ${stats.range.toFixed(2)}<br>
...
window.earthViewer = {
  ...
  toggleNormalize: () => {
    ...
    if (backendData) applyBackendDisplacement(backendData);
    console.log(`Normalization ${settings.normalize ? 'enabled' : 'disabled'}`);
  }
};
